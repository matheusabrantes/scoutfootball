...
| StatsBomb Open Data | Open event dataset | Free | Yes | Curated competitions/seasons | Not complete/current | Some tournaments only | Some tournaments only | Some tournaments only | Match/event level | Strong event model | Yes | Derivable | Derivable from events | Derivable | Derivable | Event duels | Shot-stopping depends on data | Medium | Low if attribution followed | Medium transformation | Static releases | Prototyping event calculations | 3 | Excellent teaching/test data, not broad current coverage. | [Open data](https://github.com/statsbomb/open-data), [statsbombpy](https://github.com/statsbomb/statsbombpy) |
